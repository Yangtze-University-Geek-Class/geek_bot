...
- `verify` 汇总 `branch-guard`、`core`、`console-e2e`、`lint-workflows`，任一失败、取消或跳过都算失败；不得用 `continue-on-error` 掩盖失败。免费计划的私有仓库配不了分支保护，`verify` 现在还不能设成 required check，合并前由审查者核对 CI 全绿。
