...
> 三条工作流（ci / issue-lifecycle / branch-hygiene）只做机器验证（含 control 镜像只构建不推送的检查）、PR 正文核对和合并后的清理，不部署；发布镜像的 `release.yml` 随 #7 加入。平台能力按 2026-09-25 的只读核对记录。
...
| 工作流 | 触发 | 行为 |
|---|---|---|
| `ci.yml` | PR → `main`/`stage`；push `main`/`stage`（不含 tag）；`workflow_dispatch` | `branch-guard`（`node scripts/check-branch-invariants.mjs --require-remote-refs`；来源是 `task/**` 的 PR 另跑「执行记录（notes/）」：`node scripts/note.mjs check --pr --base origin/<目标分支> --head <task 分支>`，分支名经环境变量传入；每次运行都跑「执行记录一览（运行摘要）」：`node scripts/note.mjs index --summary` 追加到 Actions 的运行摘要。这个 job 不装依赖，两个脚本只用 Node 标准库）∥ `core`（Node 读 `.nvmrc`，pnpm 读 `packageManager`：`pnpm install --frozen-lockfile` → `pnpm check` → `pnpm test` → `pnpm build`）∥ `console-e2e`（同样的 Node 与 pnpm：`pnpm install --frozen-lockfile` → `pnpm exec playwright install --with-deps chromium` → `pnpm test:e2e`；验收截图 `test-results/evidence/` 每次上传为 artifact `console-e2e-evidence`，保留 30 天；失败时另传报告 `console-e2e-report`，保留 14 天）∥ `docker`（#3：构建前断言 `app/control/Dockerfile` 的每个 `FROM` 都按 digest 钉死（引用阶段名，或带 `@sha256:<64 位十六进制>`，经 `ARG` 默认值给出的按默认值核对）；`docker build -f app/control/Dockerfile .` 只构建、不推送、不登录镜像仓库；断言 `Config.User` 不为空、不是 root，容器里 uid 不是 0，带 CMD 形式的 HEALTHCHECK；再用现场生成的一次性随机密钥起一次容器，`/readyz` 返回 200、`docker stop` 后以 0 退出，日志里有 `busy` 为 0 的「WAL 已 checkpoint，库已关闭」）∥ `lint-workflows`（actionlint 1.7.12，下载后按内联 SHA-256 校验）→ `verify` 汇总 |
...
- `verify` 汇总 `branch-guard`、`core`、`console-e2e`、`docker`、`lint-workflows`，任一失败、取消或跳过都算失败；不得用 `continue-on-error` 掩盖失败。免费计划的私有仓库配不了分支保护，`verify` 现在还不能设成 required check，合并前由审查者核对 CI 全绿。
- `docker` job 现在只覆盖 control 镜像（#3），node 镜像随 #11 加进同一个 job；`env-contract`（`pnpm check:environments`）随 #7 加入。新 job 加入时要同步写进 `verify` 的 `needs` 和汇总条件（`tests/tooling/ci-docker.test.ts` 核对 `docker` 这一项）。
- `docker` job 起容器用的密钥是这一步用 `openssl rand` 现场生成的随机值，只给那个一次性容器用，跑完即删；它不是 GitHub secrets，也不是任何实例的密钥。基础镜像按 digest 从 Docker Hub 拉取。
...
- `console-e2e`：`pnpm exec playwright install chromium && pnpm test:e2e`（见 [LOCAL-DEV](LOCAL-DEV.md)）。
- `docker`：`docker build -f app/control/Dockerfile -t geek-bot-control:ci .`，断言与起容器的步骤见 [LOCAL-DEV](LOCAL-DEV.md)「control 镜像」。
