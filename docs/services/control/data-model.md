...
- **一个环境一个库。** 容器内 `/data/geek-bot.db`，放在该环境自己的命名卷里；preview 与 production 各一个库，互不共享（挂载方式由 #3、#7 定）。
- **只有 control 进程写库。** console、node、runner、部署脚本都不打开库。CLI 怎样写库的计划做法是：会写库的子命令（例如 `bootstrap-code`、`backup`）经容器内的本地通道交给运行中的 control 执行，CLI 自己不开写连接；`restore` 和 `rotate-master-key` 只在 control 停止时以独占方式运行。这是计划，#3 定稿（通道形式由 #3、#5 定）。
- **连接参数。** `journal_mode=WAL`、`synchronous=FULL`、`foreign_keys=ON`，并设 `busy_timeout`（数值 #3 定）。
- **类型。**
...
- **库外文件**：同一个卷下的 `mirrors/`（镜像克隆）、`bundles/`（任务包）、`tasks/<任务 id>/`（事件原文 `events.jsonl.gz`、补丁等产物）、`backups/`（加密备份）。它们的保留期写在对应的表下面。库、备份和事件文件里有仓库内容和任务输出，按实例数据对待：不进仓库、不进镜像、不进日志（ADR-0008）。
...
| 列 | 类型与约束 | 说明 |
|---|---|---|
| `kind` | `TEXT NOT NULL` | 例如 `backup_verify_failed`、`node_offline`、`writes_paused`、`catalog_model_missing`、`pool_empty`、`rebuilt_from_github` |
...
| `backup_key_id` | `TEXT NOT NULL` | 加密用的备份密钥的指纹（不是密钥本身），轮换后据此找对应的离线密钥 |
...
- 放宽枚举取值：SQLite 改不了已有的 `CHECK`，要按 SQLite 官方的重建表步骤在一个事务里完成（新建表、复制数据、删旧表、改名，结束前跑 `PRAGMA foreign_key_check`）。取值只增不减，仍算只扩不缩（`shrink=false`），但必须有「上一版代码能读写新库」的测试。
...
4. D 小于 C：先做一次 `pre_migration` 备份，再按编号逐个执行。每个文件在自己的事务里执行，同一个事务里写 `schema_migrations` 并设 `PRAGMA user_version`；任何一个失败就回滚这个文件并拒绝启动，库停在上一个版本。
5. D 大于 C 而 K 不大于 C：这是回滚到上一版镜像的情形，正常启动、正常读写，不执行迁移；后台概览显示库版本比代码新。
6. 其余情况（D 等于 C）正常启动。

拒绝启动时用中文输出原因，以非 0 退出，健康检查失败，部署脚本按健康门回滚（#7）。`/readyz` 的库相关条件与此一致：库可写、K 不大于 C、D 不小于 C（没有待执行的迁移）才返回 200；所以回滚到上一版镜像时（D 大于 C、K 不大于 C）健康门能过。`/readyz` 的完整检查项见 [API](../../architecture/API.md)。上面是本文提议的表示方式，ADR-0008 把具体表示交给 #3 定；#3 用「上一版代码打开新库能启动、能读写」的测试证明。
...
- 每天一次（时刻 #3 定）；每周第一次记为 `weekly`。
- 部署前由部署脚本请求一次（`pre_deploy`，#7）；迁移前由 control 自动做一次（`pre_migration`）；管理员可以在后台或用 CLI 手动做（`manual`）。

**做法**

1. 用 better-sqlite3 的在线 backup 把库复制到临时文件，不停服务；
2. 打开临时文件，统计各表行数，写进 `row_counts_json`；
3. 用备份加密密钥（`<栈根>/secrets/backup_key`，只挂给 control，不进备份）加密，算加密后文件的 sha256；
4. 改名进 `backups/`，写一行 `backups`，删除临时的明文文件。
...
**保留**：每日备份保留 `GEEK_BOT_BACKUP_KEEP_DAILY` 份（默认 7），每周保留 `GEEK_BOT_BACKUP_KEEP_WEEKLY` 份（默认 4）；`pre_deploy`、`pre_migration`、`manual` 各保留最近 3 份（推荐值，#3 定稿）。删文件时写 `pruned_at`。
...
任何一步失败：`verify_result` 记 `failed`，写一条 `critical` 告警，后台概览显示「最近一次恢复校验失败」。

**异地副本**：可选，把加密后的备份文件复制到外置盘或 NAS 目录（配置项由 #20 定）。

**恢复**

- `restore --dry-run <文件>` 先做上面的校验，报告会恢复到哪个库版本和哪个时间点，不改任何东西。
- 正式恢复由获授权的人执行，要求 control 已停止；恢复前把当前库另存一份。
...
- #3：空库迁到最新；上一版本的库文件夹具迁到最新；上一版代码打开新库能启动、能读写；兼容版本高于代码时拒绝启动；迁移中途失败时库停在原版本；迁移文件被改动时拒绝启动；`audit_logs` 的更新和删除被触发器拒绝；备份、每日恢复校验、保留策略，以及校验失败时产生告警。
...
- 兼容版本的具体表示（本文提议记在 `schema_migrations.compat_version`，迁移文件首行声明 `shrink=`）由 #3 定稿；改了就同步本文。
- `bot_writes`、`outbox` 终态行、已解决告警的保留期没有定；`pre_deploy`、`pre_migration`、`manual` 备份各保留 3 份是推荐值。
