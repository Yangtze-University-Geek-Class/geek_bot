...
#4 建好了后台外壳（侧栏与窄屏抽屉、各类状态、样板数据模式）和浏览器回归；各业务页面随对应 issue 加入。本文的规则对任何界面改动都有效；页面清单、用到的主题令牌、图标集（UnoCSS 图标预设 + Carbon）和窄屏断点（960px）见 [console 服务文档](../services/console/README.md)。
...
- 只有浅色一套，第一版不做深色主题。
- 只经 Tuffex 官方的 `--tx-*` 令牌和组件变量换肤；组件里不写颜色值，自写 CSS 只管版式和页面底色。用到的令牌清单见 console 服务文档「主题、图标与断点」。
...
- 浏览器层（`pnpm test:e2e`，#4 加入）：页面上原生 select 与 checkbox 的数量为 0，emoji 扫描为 0，390px 宽度下没有页面级横向溢出，窄屏抽屉可用，Tab/Esc 与弹层焦点正确；覆盖范围见 console 服务文档「验证」。
