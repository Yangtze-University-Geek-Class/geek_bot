...
[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/recommendation-card) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/recommendation-card.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/recommendation-card.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
<template>
  ...
  >
    <template #body="{ option }">
      从 <code>cone_king</code> 补货华夫筒，交期 <code>7_days</code>。
    </template>
  </TxRecommendationCard>
</template>
```
...
- **选中即提升。** 在抽屉里点一条备选会把它设为当前建议、关闭抽屉，并**清空已确认态**——上一条的确认不该顺延到新方案上。
- 抽屉只列**其他**方案，当前建议不会重复出现在里面。
...
- 把 `accepted` 提到宿主并在请求成功后再置 true，否则用户会以为已经下单。
...
- **与 `TxToolConfirmation` 的分工:** 那是二元授权（允许 / 拒绝 + 风险分级），本组件是**带证据的多选建议**：有方案列表、置信度与提升语义。只有页脚两颗按钮的外形相似。
- **量表是独立原语:** 页脚与抽屉每行用的都是 `TxSignalMeter`，可以单独引入，见它自己的文档。
- **可访问性:** 收起抽屉的 `inert` 是对上游缺陷的修正。
- **已知偏差:** 主按钮的 `box-shadow` 在上游明暗两套主题里是同一组 `rgba(16,24,40,·)` 硬编码值。这里原样保留以对齐截图——它读起来是实心填充上的一层高光，而不是随主题走的发丝线。
...
- [recommendation-card/index.ts](../snapshot/packages/tuffex/packages/components/src/recommendation-card/index.ts.txt)
- [src/TxRecommendationCard.vue](../snapshot/packages/tuffex/packages/components/src/recommendation-card/src/TxRecommendationCard.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/recommendation-card/src/types.ts.txt)
