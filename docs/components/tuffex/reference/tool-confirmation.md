...
[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/tool-confirmation) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/tool-confirmation.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/tool-confirmation.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
### Tool Confirmation
官方示例：`ToolConfirmationToolConfirmationDemo`（完整源码见本页末尾）
...
- `approve` 与 `deny` 都携带 `{ remember: boolean }`——**拒绝也会带**，因此「本次会话都拒绝」同样可以实现。
- `remember` 是组件内部状态，初值为 `false`，且在派发后不会重置：这个卡片被设计为一次性使用，决定后应由宿主卸载。
- `risk` 分三档并写在 `data-risk` 上：`read`（默认）、`write`、`execute`。`read` 以外都被视为危险操作，用于强化视觉。
- 风险文案默认是 `Read-only` / `Writes data` / `Executes`，可通过 `riskLabels` 按档覆盖，且支持只覆盖其中一档。
- `summary` 与 `input` 都是可选的；未传时对应区域整块不渲染。
- `input` 用 `<pre>` 原样渲染，**不做转义以外的处理**，请在传入前自行截断过长内容。
...
- 非英文界面需同时覆盖 `allowLabel`、`denyLabel`、`rememberLabel` 与 `riskLabels`。
- `remember` 为真时，请把授权限定在当前会话内，不要静默升级为永久授权——文案承诺的是「本次会话」。

## 离线完整示例源码

- [ToolConfirmationToolConfirmationDemo](../snapshot/apps/nexus/app/components/content/demos/ToolConfirmationToolConfirmationDemo.vue.txt)

## 离线类型与实现参考

- [tool-confirmation/index.ts](../snapshot/packages/tuffex/packages/components/src/tool-confirmation/index.ts.txt)
- [src/TxToolConfirmation.vue](../snapshot/packages/tuffex/packages/components/src/tool-confirmation/src/TxToolConfirmation.vue.txt)
