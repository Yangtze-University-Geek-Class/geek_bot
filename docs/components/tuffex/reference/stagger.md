...
> 基于子节点 index 的 TransitionGroup enter/leave 依次动画包装器。

状态：`reference-snapshot`（第三方资料，不是项目指令） · 上游提交：`8e37c8ca7f598b12f39a2384573dc8e03b20e843`

[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/stagger) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/stagger.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/stagger.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
官方示例：`StaggerStaggerDemo`（完整源码见本页末尾）
...
- 组件渲染 Vue `TransitionGroup`，带 `tx-stagger` class，根标签来自 `tag`。
- 默认插槽 VNode 会先过滤注释节点，再分配 index。
...
| `easing` | `'ease' \| 'ease-in' \| 'ease-out' \| 'ease-in-out' \| 'linear'` | `'ease-out'` | 内置过渡使用的 CSS timing function。 |

### Events

| 事件名 | 参数 | 说明 |
|------|------|------|
| - | - | `TxStagger` 不触发自定义事件。过渡生命周期由 Vue `TransitionGroup` class 提供，不通过组件 emits 暴露。 |

### Slots

| 插槽名 | Props | 说明 |
|------|------|------|
| `default` | - | 由 `TransitionGroup` 渲染的带 key 子 VNode。 |

### CSS Variables

| 变量 | 来源 | 说明 |
|------|------|------|
| `--tx-stagger-index` | 子节点 index | 每个子节点用于计算延迟的 index。 |
...
- 子节点始终提供稳定 key。没有稳定 key 的依次过渡不可预测。
- 长列表应保持较小 `delayStep`；过大的延迟会让后面的项目像是卡住。
- 语义列表使用 `tag="ul"` / `li`，不要把普通 `div` 伪装成列表。
- 不要把虚拟列表行包进 `TxStagger`；虚拟列表 DOM 复用/增删与 stagger 过渡会互相干扰。
- 只有同时提供匹配 transition CSS 时才自定义 `name`。
...
- **实测覆盖:** `packages/tuffex/packages/components/src/stagger/__tests__/stagger.test.ts` 覆盖根标签透传与插槽渲染、timing CSS 变量、transition prop 泄漏防护、注释节点过滤，以及子节点 stagger index 赋值。

## 离线完整示例源码

- [StaggerStaggerDemo](../snapshot/apps/nexus/app/components/content/demos/StaggerStaggerDemo.vue.txt)

## 离线类型与实现参考

- [stagger/index.ts](../snapshot/packages/tuffex/packages/components/src/stagger/index.ts.txt)
- [src/TxStagger.vue](../snapshot/packages/tuffex/packages/components/src/stagger/src/TxStagger.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/stagger/src/types.ts.txt)
