...
[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/search-panel) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/search-panel.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/search-panel.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
- `TxCommandPalette` 是**模态**——Teleport 到 body、全屏遮罩、`role="dialog" aria-modal="true"`、焦点陷阱、z-index 分配。它接管整个页面。
- `TxSearchPanel` 是**内联**——没有遮罩、没有模态语义、没有焦点陷阱。它可以放进页面、侧边栏或浮层里，与周围内容共存。
...
- **键盘导航是补齐的能力，上游完全没有。** ↑ ↓ 在两端循环并跳过禁用项，Home / End 跳到首尾可用项，Enter 选中当前高亮项，Esc 在有内容时清空。输入法组合期间的 Enter 被忽略（同时判定 `isComposing` 与 `compositionstart` / `compositionend`），不会把候选词确认误当成选中。
- ARIA 走 combobox 范式：输入框是 `role="combobox"` + `aria-autocomplete="list"` + `aria-controls` + `aria-activedescendant`，列表是 `role="listbox"`，每行是 `role="option"` 且 `tabindex="-1"`——焦点始终留在输入框，Tab 会离开整个部件而不是逐行穿过。
...
- **复用判断:** 空态复用了 `TxSearchEmpty`；输入行没有复用 `TxSearchInput`——后者经由 `TuffInput` 携带 `fake-background` 类，会在着色主题下被套上第二层内环，且其 32px / 12px 圆角 / 主色聚焦环的观感与本卡片的无边框 40px 行相去甚远。
- **偏离上游:** 补齐键盘导航与 ARIA，选中不回写输入框。前者上游为零，后者上游是 demo 手法。
- **可访问性:** 结果行虽是 `<button>`，但以 `tabindex="-1"` 排除在 Tab 序列之外，这是 combobox 范式的要求，不是疏漏。
...
- [search-panel/index.ts](../snapshot/packages/tuffex/packages/components/src/search-panel/index.ts.txt)
- [src/TxSearchPanel.vue](../snapshot/packages/tuffex/packages/components/src/search-panel/src/TxSearchPanel.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/search-panel/src/types.ts.txt)
