...
[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/approval-card) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/approval-card.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/approval-card.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
`TxToolConfirmation` 是一次二元授权：某个工具调用允不允许，输出 approve / deny 和风险分级。本组件是**问卷**：智能体动手前先问清楚几件事，输出一组答案。两者的数据模型与输出契约没有交集，不要互相替代，也不要把问卷塞进授权卡。
...
- 发送键在「有选中项**或**自由文本非空」时才可用；它在末题是提交，在其他题是前进。
- 分页圆点填充表示**该题已作答**，不是「已经过」。上游按 `i < 当前题` 判断，往回翻时会把已答的题显示成未答，这里做了修正。
...
- 关闭后整卡换成一颗按钮，内部状态不清空；`reset()` 或 `startOver` 才会清。
- 题目按钮是 `button` + `aria-pressed`，整组套 `role="group"` 并 `aria-labelledby` 指向题干，读屏能拿到题目上下文。

## 最佳实践

- 接 `answer` 做增量落库，别只等 `submit`——用户中途关掉卡片时前面几题的答案不该丢。
...
- **与 `TxToolConfirmation` 的分工:** 见上文「它不是授权闸门」。名字相近但语义正交，文档里必须一起读。
- **可访问性:** `role="group"` + `aria-labelledby` 指向题干是有意超出上游的修正；上游只有裸的 `aria-pressed`，读屏拿不到题目上下文。
- **已知偏差:** 上游的自动翻页在减少动效下照常触发（它只有一条全局 `*` 规则压缩时长）。这里改成抑制，理由写在交互契约里；若宿主需要上游行为，显式传 `autoAdvance` 并自行判断偏好。
- **已知偏差:** 上游按数组下标存答案，本组件改用 `option.value`。
...
- [approval-card/index.ts](../snapshot/packages/tuffex/packages/components/src/approval-card/index.ts.txt)
- [src/TxApprovalCard.vue](../snapshot/packages/tuffex/packages/components/src/approval-card/src/TxApprovalCard.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/approval-card/src/types.ts.txt)
