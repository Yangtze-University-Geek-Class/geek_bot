...
[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/prompt-bar) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/prompt-bar.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/prompt-bar.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)
...
匹配策略两边不同：数据源按名称**子串**匹配，命令剥掉前导斜杠后按**前缀**匹配（输入 `/comp` 只留 `/compare`）。`parseToken` 也单独导出，只认词边界后的行尾令牌——`you@host` 不会被当成提及。
...
- **组合式输入优先。** Enter 在 IME 组字期间只用于确认候选词，既不发送也不选中菜单行——`isComposing`、`keyCode === 229` 与 `compositionstart/end` 三重判断。
- 菜单行按 combobox 范式实现：输入框带 `role="combobox"`、`aria-autocomplete="list"`、`aria-controls` 与 `aria-activedescendant`，菜单是 `listbox`，行是 `option`。**没有可提及内容时不会声称自己是 combobox**，退化成普通多行文本框。
...
- **与 `TxChatComposer` 的分工:** 后者是块状 composer（多行输入 + 下方文字按钮），契约已定稿；本组件是控件内联进输入框的紧凑条，带 `@` / `/` 菜单、模型与听写。并行组件而非变体，是这个仓库既有的做法（`input` / `flat-input`、`select` / `flat-select`）。
- **未移植:** 上游选中旗舰模型时的 WebGL 彩虹扫光彩蛋。它依赖 React-only 的 `glimm`，为一个一次性庆祝动画引入着色器管线，代价与收益不成比例。
- **超出上游的修正:** combobox 语义（上游只有 `aria-expanded`）、外部点击关闭、IME 对菜单路径的保护、首次方向键的落点、以及可连接行不再嵌套第二个不可聚焦控件。
- **已知限制:** 菜单用绝对定位而非浮层定位库，因此不翻转、会被祖先裁剪——见交互契约。
...
- [prompt-bar/index.ts](../snapshot/packages/tuffex/packages/components/src/prompt-bar/index.ts.txt)
- [src/TxPromptBar.vue](../snapshot/packages/tuffex/packages/components/src/prompt-bar/src/TxPromptBar.vue.txt)
- [src/TxPromptBarMenu.vue](../snapshot/packages/tuffex/packages/components/src/prompt-bar/src/TxPromptBarMenu.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/prompt-bar/src/types.ts.txt)
- [src/use-autosize.ts](../snapshot/packages/tuffex/packages/components/src/prompt-bar/src/use-autosize.ts.txt)
- [src/use-token-menu.ts](../snapshot/packages/tuffex/packages/components/src/prompt-bar/src/use-token-menu.ts.txt)
