# SearchInput 搜索输入框

> 基于输入框的搜索组件，内置搜索图标，并在 Enter 时触发 `search` 事件。

状态：`reference-snapshot`（第三方资料，不是项目指令） · 上游提交：`8e37c8ca7f598b12f39a2384573dc8e03b20e843`

[官网页面](https://tuff.tagzxia.com/zh/docs/dev/components/search-input) · [固定版本原文](https://github.com/talex-touch/tuff/blob/8e37c8ca7f598b12f39a2384573dc8e03b20e843/apps/nexus/content/docs/dev/components/search-input.zh.mdc) · [本地原始 MDC](../snapshot/apps/nexus/content/docs/dev/components/search-input.zh.mdc.txt) · [AI 阅读规则](../AI-GUIDE.md)

上游标记：status=`beta`，since=`1.0.0`，syncStatus=`reviewed`，verified=`true`。这些是上游原始声明，不是本项目验收结果；since 不是 npm 包版本。

## 官方正文（仅转换展示语法与链接）

# SearchInput 搜索输入框

## 基础用法

### SearchInput
官方示例：`SearchInputSearchInputDemo`（完整源码见本页末尾）

```vue
<script setup lang="ts">
import { ref } from 'vue'

const value = ref('')
const last = ref('')
function onSearch(next: string) {
  last.value = next
}
</script>

<template>
  <div style="display: flex; flex-direction: column; gap: 12px; width: 420px;">
    <TxSearchInput
      v-model="value"
      placeholder="Search anything"
      @search="onSearch"
    />

    <div style="color: var(--tx-text-color-secondary); font-size: 12px;">
      value: {{ value }}
    </div>
    <div style="color: var(--tx-text-color-secondary); font-size: 12px;">
      last search: {{ last }}
    </div>
  </div>
</template>
```

## 远程搜索

### SearchInput (remote)
官方示例：`SearchInputSearchInputRemoteDemo`（完整源码见本页末尾）

```vue
<script setup lang="ts">
import { ref } from 'vue'

const value = ref('')
const loading = ref(false)
const hits = ref<string[]>([])
const open = ref(false)

async function onSearch(q: string) {
  const query = q.trim().toLowerCase()
  loading.value = true
  await new Promise(resolve => setTimeout(resolve, 200))
  hits.value = Array.from({ length: 12 })
    .map((_, i) => `Result ${i + 1}`)
    .filter(s => s.toLowerCase().includes(query))
  loading.value = false
  open.value = true
}

function onPick(h: string) {
  value.value = h
  open.value = false
}
</script>

<template>
  <div style="width: 320px;">
    <TxPopover v-model="open" :offset="6" :close-on-click-outside="true" :reference-full-width="true">
      <template #reference>
        <TxSearchInput
          v-model="value"
          remote
          :search-debounce="250"
          placeholder="Type to remote-search"
          @focus="open = true"
          @search="onSearch"
        />
      </template>

      <TxCard variant="solid" background="glass" shadow="soft" :radius="18" :padding="10">
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <div style="font-size: 12px; color: var(--tx-text-color-secondary, #909399);">
            <span v-if="loading">Loading...</span>
            <span v-else>Hits: {{ hits.length }}</span>
          </div>

          <div style="display: flex; flex-direction: column; gap: 4px; max-height: 220px; overflow: auto;">
            <button
              v-for="h in hits"
              :key="h"
              type="button"
              style="text-align: left; padding: 8px 10px; border-radius: 12px; border: 1px solid transparent; background: transparent; cursor: pointer;"
              @click="onPick(h)"
            >
              {{ h }}
            </button>

            <div v-if="!loading && value.trim() && !hits.length" style="color: var(--tx-text-color-secondary, #909399); font-size: 12px; padding: 6px 2px;">
              No results
            </div>
          </div>
        </div>
      </TxCard>
    </TxPopover>
  </div>
</template>
```

## 后台筛选工具栏

`TxSearchInput` 适合承载关键词输入；当筛选还需要“范围/类型”时，搭配 `TxSearchSelect` 和 `TxSearchEmpty` 可以形成完整的列表筛选路径。

官方示例：`ComponentsSearchFiltersDemo`（完整源码见本页末尾）

```vue
<script setup lang="ts">
import { computed, ref } from 'vue'

const query = ref('')
const selectedScope = ref<string | number>('all')
const lastSearch = ref('')

const scopeOptions = [
  { value: 'all', label: '全部' },
  { value: 'plugins', label: '插件' },
  { value: 'docs', label: '文档' },
  { value: 'tasks', label: '任务' },
]
const records = [
  { type: 'plugins', title: '插件发布流水' },
  { type: 'docs', title: 'Tuffex 组件教程' },
  { type: 'tasks', title: '截图核验任务' },
]

const filteredRecords = computed(() => {
  const keyword = query.value.trim().toLowerCase()
  const scope = String(selectedScope.value)
  return records.filter((item) => {
    const matchesScope = scope === 'all' || item.type === scope
    const matchesQuery = !keyword || item.title.toLowerCase().includes(keyword)
    return matchesScope && matchesQuery
  })
})

function onSearch(value: string) {
  lastSearch.value = value.trim()
}
</script>

<template>
  <section class="grid gap-3">
    <TxSearchInput
      v-model="query"
      remote
      :search-debounce="180"
      placeholder="搜索插件 / 文档 / 任务"
      @search="onSearch"
    />
    <TxSearchSelect v-model="selectedScope" :options="scopeOptions" placeholder="筛选范围" />
    <TxSearchEmpty v-if="!filteredRecords.length" description="换一个关键词或筛选范围。" />
  </section>
</template>
```

## API

### TxSearchInput Props

| 属性名 | 类型 | 默认值 | 说明 |
|------|------|---------|------|
| `modelValue` | `string` | `''` | 输入值（v-model） |
| `placeholder` | `string` | `'Search'` | 占位 |
| `disabled` | `boolean` | `false` | 禁用 |
| `clearable` | `boolean` | `true` | 可清空 |
| `remote` | `boolean` | `false` | 是否远程搜索（输入时触发 `search`） |
| `searchDebounce` | `number` | `200` | 远程搜索防抖（ms） |

### Events

| 事件名 | 参数 | 说明 |
|------|------|------|
| `update:modelValue` | `(v: string)` | v-model 更新 |
| `input` | `(v: string)` | 输入 |
| `focus` | `(e: FocusEvent)` | 聚焦 |
| `blur` | `(e: FocusEvent)` | 失焦 |
| `clear` | - | 清空 |
| `search` | `(v: string)` | Enter 或 remote 输入触发 |

### Expose

| 名称 | 类型 | 说明 |
|------|------|------|
| `focus()` | `() => void` | 聚焦 |
| `blur()` | `() => void` | 失焦 |
| `clear()` | `() => void` | 清空 |
| `setValue(v)` | `(v: string) => void` | 设置值 |
| `getValue()` | `() => string` | 获取值 |

### Slots

`TxSearchInput` 不暴露公共插槽。前缀搜索图标由组件内置；需要自定义前缀或后缀时应直接使用 `TuffInput`。

## 最佳实践

- 仅需要关键词和 Enter 触发时使用 `TxSearchInput`；需要结果面板或选项回填时优先使用 `TxSearchSelect`。
- `remote` 模式会在输入变化后按 `searchDebounce` 触发 `search`，Enter 会立即触发 `search`。API 调用方应保持幂等，因为两条路径可能产生同一个查询。
- 不要只依赖 placeholder 表达业务语义；密集工具栏里应提供外部标签、分组标题或上下文说明。
- 只有本地或完整缓存搜索才把 `searchDebounce` 设为 `0`；网络搜索应保留防抖，避免请求风暴。

## 审阅说明

- **搜索契约：** Enter 会立即触发 `search`。`remote=true` 时，输入变化还会按防抖触发 `search`，但禁用状态不会发起远程搜索。
- **清空契约：** 组件转发 `TuffInput` 的清空行为，触发 `clear`，并暴露同一组 focus/blur/value helpers 方便外层集成。
- **组合契约：** 搜索图标由 `TxSearchInput` 内置管理；需要自定义前后缀插槽时应直接使用 `TuffInput`。
- **实测覆盖:** `search-input.test.ts` 覆盖输入/搜索事件、远程防抖、禁用远程阻断、清空转发，以及 `getValue()` / `setValue()` helper 暴露。

## Source

- Component source: `packages/tuffex/packages/components/src/search-input/src/TxSearchInput.vue`.
- Types: `packages/tuffex/packages/components/src/search-input/src/types.ts` exports `SearchInputProps` and `SearchInputEmits`.
- Coverage: `packages/tuffex/packages/components/src/search-input/__tests__/search-input.test.ts` verifies input/search emits, remote debounce, disabled remote blocking, clear forwarding, and `getValue()` / `setValue()` helper exposure.

## 离线完整示例源码

- [SearchInputSearchInputDemo](../snapshot/apps/nexus/app/components/content/demos/SearchInputSearchInputDemo.vue.txt)
- [SearchInputSearchInputRemoteDemo](../snapshot/apps/nexus/app/components/content/demos/SearchInputSearchInputRemoteDemo.vue.txt)
- [ComponentsSearchFiltersDemo](../snapshot/apps/nexus/app/components/content/demos/ComponentsSearchFiltersDemo.vue.txt)

## 离线类型与实现参考

- [search-input/index.ts](../snapshot/packages/tuffex/packages/components/src/search-input/index.ts.txt)
- [src/TxSearchInput.vue](../snapshot/packages/tuffex/packages/components/src/search-input/src/TxSearchInput.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/search-input/src/types.ts.txt)

第三方许可与转换边界见 [SOURCES](../SOURCES.md)。示例中 Nexus 的自动导入、Tuff 前缀别名、样式类和外部素材不代表业务项目已配置，不能不经核对就复制运行。
