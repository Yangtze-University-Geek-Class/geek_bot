...
### VersionCapsule
官方示例：`VersionCapsuleVersionCapsuleDemo`（完整源码见本页末尾）
...
const notice = {
  ...
  points: ['功能可能随时调整', '建议仅尝鲜用户使用'],
}
const builds = [
  { id: 'mac-arm', name: 'macOS · Apple 芯片', meta: '.dmg · 126 MB', href: '#mac-arm', recommended: true },
  { id: 'win', name: 'Windows · x64', meta: '.exe · 118 MB', href: '#win' },
]
const latest = { id: '1', tag: 'v2.4.13-beta.19', channel: 'BETA', tone: 'preview', date: '7 月 21 日', note: '图标自愈' }
const entries = [
  { id: '2', tag: 'v2.4.12', channel: 'RELEASE', tone: 'stable', date: '7 月 12 日', href: '#v2412' },
]
</script>
...
<template>
  <TxVersionCapsule v-model:panel="panel" version="v2.4.13-beta.19" channel="BETA" tone="preview">
    ...
    <template #history="{ close }">
      <TxVersionHistoryPanel :latest="latest" :entries="entries" count-label="共 6 个版本" @select="close" />
    </template>
  </TxVersionCapsule>
</template>
```
...
官方示例：`VersionCapsuleChannelTonesDemo`（完整源码见本页末尾）

```vue
<script setup lang="ts">
const capsules = [
  { key: 'stable', tone: 'stable', version: 'v2.4.12', channel: '稳定版', historyTitle: '版本历史', latest: { id: 's1', tag: 'v2.4.12', channel: '稳定版', tone: 'stable', date: '6 月 23 日', note: '已完成签名与回归验证的正式构建。' } },
  { key: 'preview', tone: 'preview', version: 'v2.4.13-beta.19', channel: '测试版', historyTitle: '版本历史', latest: { id: 'p1', tag: 'v2.4.13-beta.19', channel: '测试版', tone: 'preview', date: '7 月 21 日', note: '面向尝鲜用户的预发布通道。' } },
  { key: 'nightly', tone: 'nightly', version: 'v2.4.14-nightly.5', channel: '每日构建', historyTitle: '版本历史', latest: { id: 'n1', tag: 'v2.4.14-nightly.5', channel: '每日构建', tone: 'nightly', date: '今天', note: '自动构建，可能不稳定。' } },
  { key: 'neutral', tone: 'neutral', version: 'v1.9.8', channel: '已归档', historyTitle: '版本历史', latest: { id: 'a1', tag: 'v1.9.8', channel: '已归档', tone: 'neutral', date: '2024', note: '不再维护的历史版本。' } },
]
</script>
...
<template>
  <div style="display: flex; flex-direction: column; gap: 14px; align-items: flex-start;">
    ...
    >
      <template #history="{ close }">
        <TxVersionHistoryPanel :title="item.historyTitle" :latest="item.latest" @select="close" />
      </template>
    </TxVersionCapsule>
  </div>
</template>
...
| `recommended` | `boolean?` | 标记访客所在平台的构建。仅**第一个**推荐构建会被高亮并获得带文案的下载按钮，其余回退为纯图标。 |
...
不传某个插槽时，该分段退化为**纯触发器**：仍然抛出对应事件，但不弹出任何面板。
...
- 两个分段是**面向不同任务的独立触点**：左半下载构建，右半打开历史。它们是各自独立的点击目标——不要再把整个胶囊做成一个动作。
- 同一时刻只打开一个面板。点击另一个分段会切换面板；再次点击当前分段则关闭。
- 面板锚定在各自分段下方（下载 → 左对齐，历史 → 右对齐），刻意**不是**模态。
- `panel` 是可选的。省略即为非受控，或用 `v-model:panel`（或 `:panel` + `@update:panel`）驱动。`undefined` 表示"未受控"；`null` 是有效的受控值，表示"全部关闭"。
- `closeOnClickOutside` 与 `closeOnEsc` 控制关闭方式；`close()`（也会传入两个插槽）在关闭的同时把焦点还给触发的分段。
- 设置 `disabled` 会关闭已打开的面板并阻断两个分段。
- `notice` 位于下载面板内、构建列表之上：预发布警告是**下载路径上的知情同意**，而非被动的 hover 提示。
...
- 让胶囊**配合**而非取代主认证版按钮：按钮交付稳定构建，胶囊承载预览通道（下载 + 历史）。
...
- 类型契约：`packages/tuffex/packages/components/src/version-capsule/src/types.ts` 导出 `TxVersionCapsuleProps`、`TxVersionDownloadPanelProps`、`TxVersionHistoryPanelProps`、`TxVersionBuild`、`TxVersionNotice`、`TxVersionHistoryEntry` 和 `TxVersionChannelTone`。
...
- [VersionCapsuleVersionCapsuleDemo](../snapshot/apps/nexus/app/components/content/demos/VersionCapsuleVersionCapsuleDemo.vue.txt)
- [VersionCapsuleChannelTonesDemo](../snapshot/apps/nexus/app/components/content/demos/VersionCapsuleChannelTonesDemo.vue.txt)

## 离线类型与实现参考

- [version-capsule/index.ts](../snapshot/packages/tuffex/packages/components/src/version-capsule/index.ts.txt)
- [src/TxVersionCapsule.vue](../snapshot/packages/tuffex/packages/components/src/version-capsule/src/TxVersionCapsule.vue.txt)
- [src/TxVersionDownloadPanel.vue](../snapshot/packages/tuffex/packages/components/src/version-capsule/src/TxVersionDownloadPanel.vue.txt)
- [src/TxVersionHistoryPanel.vue](../snapshot/packages/tuffex/packages/components/src/version-capsule/src/TxVersionHistoryPanel.vue.txt)
- [src/types.ts](../snapshot/packages/tuffex/packages/components/src/version-capsule/src/types.ts.txt)
