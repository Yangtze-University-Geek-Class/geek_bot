...
状态：`proposed` · 更新：2026-09-26 · 适用：`app/console` 的技术选型与构建产物的托管方式（由 #4 实现，同源托管随 #3、#7 的 control 镜像落地）
...
待定：在 #2 的 PR 上给出。推荐按本 ADR 执行（#22 的 18 项待定事项里没有对应条目）。

- 用 Tuffex 是所有者的硬性要求，已经写进 Tuffex 使用政策；需要所有者给结论的是「由 control 同源托管、不另起静态服务器」和「版本钉死在 0.6.0、升级单独立项」。
