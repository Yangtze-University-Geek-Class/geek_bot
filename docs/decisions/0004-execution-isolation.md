...
状态：`proposed` · 更新：2026-09-26 · 适用：`app/node` 的 sandbox 与 VM 执行器、`app/runner` 的运行环境、node 栈的容器配置（由 #12 实测，#14、#17 实现）
...
待定：在 #2 的 PR 上给出。推荐按本 ADR 执行（对应 #22 第 6 项；VM 就绪前 PR 审查先在只读 sandbox 上线对应第 5 项）。
