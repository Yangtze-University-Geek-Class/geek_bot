...
状态：`proposed` · 更新：2026-09-26 · 适用：`app/control` 的进程与写入模型、节点协议 `/api/node/v1`、`app/node` 与 control 的连接方式（由 #3、#9、#11 实现，多节点见 #19）
...
待定：在 #2 的 PR 上给出。推荐按本 ADR 执行（对应 #22 第 8 项）。
