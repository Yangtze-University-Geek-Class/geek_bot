...
10. **危险操作**：数据库结构变更是否有兼容与恢复路径（迁移只扩不缩，见 ADR-0008，#3 起由迁移器强制）；是否有删除数据、覆盖配置、顺带升级无关依赖、修改生产凭据的动作；是否存在「以测试通过代替人工验收」的表述；是否绕过或放宽 CI 与校验（`|| true`、`continue-on-error`、`[skip ci]`、删断言、改校验器、放宽既有校验来换绿色），出现即阻塞。
