...
| `blocked` | 阻塞 | 需要人决定、缺凭据、依赖别的 issue | issue（同时 @ 相关人；机器人只 @ 当前条目的作者，见 §6） |
...
- 产品每条写入另带一行隐藏标记 `<!-- geek-bot v1 ... -->`，用于幂等和从 GitHub 重建状态（[ADR-0005](../decisions/0005-rules-from-base-branch.md)）。它是产品标识，不是追踪规范：解析追踪记录只看记录头，两者不能互相替代。
...
哪些 issue 由机器人接、标签怎么用见 [ISSUES](ISSUES.md) §5；追问轮数、提醒与关闭天数、优先级等完整默认行为见 [默认行为](../services/control/behavior.md)。
