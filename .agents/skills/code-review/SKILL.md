...
15. **执行记录**：
    ```bash
    node scripts/note.mjs check --pr --for-review --base origin/stage --head <task 分支>
    git diff origin/stage...HEAD --stat -- notes
    git diff origin/stage...HEAD -- notes ':(exclude)notes/INDEX.md' | grep -nE '^-' | grep -vE '^[0-9]+:--- (a/|/dev/null)'
    node scripts/note.mjs index --summary
    ```
    打开 `notes/<日期>/<用户名>/<链路>.md`，对照 `git log`、PR 和 CI 运行号，核对记录的时间、SHA、命令输出与实际一致，没有补写没发生过的事；第三条命令有输出（`INDEX.md` 以外）说明改写或删除了已有记录，即阻塞。逐句看记录内容：除负责人的 GitHub 用户名外不得有真实账号、组织名、内部主机或网段（`notes/` 不做被禁词比对，见 `docs/conventions/NOTES.md` §7）。审查中允许暂缺「审查」记录，审查给出结论后由作者补记，CI 恢复全绿后才能合并。
...
- 第 1–15 项逐项给出结果；不涉及的项写「不涉及」，不能省略。
...
逐项：1 通过；2 通过；3 通过；4 不涉及；…；11 不涉及；12 不涉及；13 不涉及；14 不涉及；15 通过。
