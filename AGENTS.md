...
- **env 模板只放占位符和通用默认值**：实例的真实非密值（origin、主机、端口、路径）只在目标机的 `.env.<环境>` 里；**密钥一律用 `*_FILE` 指向目标机上的密钥文件，模板里的密钥项只能留空或写 `*_FILE` 路径**。这是对 ADR-0001 背景里的规范体系「非密钥项预填真实值、由 CI 用环境级 secrets 填充」的有意偏离（来源与理由见 [ADR-0001](docs/decisions/0001-standalone-product.md)）：仓库以后可能公开，免费计划的私有仓库也没有 Environments。#22 第 3 项按推荐推进（所有者未提出修改），#1 的 PR 里再请所有者确认一次。
